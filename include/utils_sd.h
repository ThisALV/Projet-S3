/// \file
/// \brief Fonctions utilitaires pour utiliser les structures de donnees basiques
...
/// \brief Tableau d'entiers a taille dynamique
...
/// \brief Matrice carree d'entiers a dimension dynamique
...
/// \brief Tableau de chaines de caracteres a taille dynamique
...
/// \brief Matrice de chaines de caracteres a dimension dynamique
...
/// \brief Arc pondere, defini par sa source, sa destination et son poids
struct s_arc_p {
...
/// \brief Liste des arcs initialement presents a l'interieur d'un graphe et pouvant
...
/// \brief Candidat pour lequel un electeur peut voter
struct s_candidat {
...
/// \brief Liste des candidats participant a une election
struct s_candidats {
...
/// \brief Cellule contenant un element de `t_liste_simple_int` et l'element suviant dans la liste
...
/// \brief Liste simplement chainee d'entiers a usage generaliste
...
/// \brief Creer un tableau d'entiers dynamique
...
/// \brief Creer une matrice carree d'entiers dynamique
...
/// \brief Desalloue une matrice carree d'entiers dynamique
/// \param[inout] mat Memoire a desallouer
/// \param[in] dim Dimension de la matrice carree
void detruire_mat_int(int** mat, int dim);

/// \brief Creer un tableau de chaines de caracteres dynamique
/// \param[in] dim Taille du tableau cree
/// \return Pointeur vers un tableau dynamique a la dimension dim ou NULL en cas d'erreur
char** creer_tab_char_star(int dim);

/// \brief Creer une matrice de chaines de caracteres dynamique
...
/// \brief Desalloue une matrice de chaines de caracteres dynamique
/// \param[inout] mat Memoire a desallouer
/// \param[in] lignes Haute de la matrice
void detruire_mat_char_star(char*** mat, int lignes);

/// \brief Creer un tableau de candidats dynamique
/// \param[in] dim Taille du tableau cree
/// \return Pointeur vers un tableau dynamique a la dimension dim ou NULL en cas d'erreur
t_candidat* creer_tab_candidats(int dim);

/// \brief Creer un tableau d'arcs ponderes dynamique
/// \param[in] dim Taille du tableau cree
/// \return Pointeur vers un tableau dynamique a la dimension dim ou NULL en cas d'erreur
...
/// \brief Detruit proprement le tableau d'entiers dynamique en desallouant sa memoire
...
/// \brief Met un tableau d'entiers non initalise en mode erreur
/// \param[out] tab Tableau a mettre en mode erreur
void mettre_t_tab_int_dyn_erreur(t_tab_int_dyn* tab);

/// \brief Verifie si un tableau est en mode erreur
...
/// \brief Detruit proprement la matrice d'entiers dynamique en desallouant sa memoire
...
/// \brief Met une matrice d'entiers non initalisee en mode erreur
/// \param[out] mat Matrice a mettre en mode erreur
void mettre_t_mat_int_dyn_erreur(t_mat_int_dyn* mat);

/// \brief Verifie si une matrice est en mode erreur
...
/// \brief Detruit proprement le tableau de chaines de caracteres dynamique en desallouant sa memoire
...
/// \brief Met un tableau de chaines de caracteres non initalise en mode erreur
/// \param[out] tab Tableau a mettre en mode erreur
...
/// \brief Detruit proprement la matrice de chaines de caracteres dynamique en desallouant sa memoire
...
/// \brief Met une matrice de chaines de caracteres non initalisee en mode erreur
/// \param[out] mat Matrice a mettre en mode erreur
...
/// \brief Detruit proprement le tableau de candidats en desallouant sa memoire
...
/// \brief Met un tableau de candidats non initalise en mode erreur
/// \param[out] tab Tableau a mettre en mode erreur
...
/// \brief Creer un tableau d'arcs dynamique
/// \param[in] dim Nombre d'arcs, la taille du tableau
/// \param[out] tab Tableau a initialiser
/// \return `true` si le tableau a bien ete initialise
bool creer_t_arcs_dyn(t_arcs* tab, int dim);

/// \brief Detruit proprement le tableau d'arcs en desallouant sa memoire
/// \param[inout] tab Tableau a desallouer, la taille sera mise a 0 et le pointeur ira vers NULL
void detruire_t_arcs_dyn(t_arcs* tab);

/// \brief Creer une liste simplement chainee d'entiers
/// \param[out] liste Liste a initialiser
void creer_t_liste_simple_int(t_liste_simple_int* liste);

/// \brief Detruit proprement la liste en desallouant la memoire de chaque cellule
...
/// \brief Insere un entier dans une cellule qui sera le nouveau debut de la liste
...
/// \brief Supprime de la l'element contenant la valeur donnee
...
/// \brief Retire et obtient le 1er element de la liste. Effectuer cette operation sur
...
/// \brief Verifie si la valeur donnee existe dans la liste
